...
class Trajectory_Optimization(Env):
    ...
    def __init__(self, initial_pos, planning_horizon, desired_coordinates, joint_vel=np.zeros(3)):
        ...
        self.desired_states = np.hstack(
            (self.desired_joint_pos, self.desired_joint_vel))
        ...
        self.K = np.zeros(
            (planning_horizon, self.actions_space, self.states_space))
        ...
        print(
            "gettting initial trajetory states from initial guess of actions (Torques) \n")
    ...
    def get_trajectory(self, alpha=1):
        ...
        for n in range(self.planning_horizon-1):
            ...
            action = self.actions[n, :] + self.K[N-(n+1)]@(np.transpose(
                updated_states[n, :]-self.states[n, :])) + alpha*self.k[N-(n+1)].ravel()

            self.actions[n, :] = action.copy()

            updated_states[n+1,
                           :] = self.get_next_state(updated_states[n, :], action)
    ...
    def dynamics(self, states: np.ndarray, action):
        ...
        qddot = self.get_joint_acceleration(
            mass_matrix, non_linear_terms, action)
    ...
    def compute_gradients(self, state, action, n):
        ...
        c_t = 2 * \
            cost_matrix @ np.hstack((state - self.desired_states, action)).T
        ...
        ddq_dq, ddq_dqdot, ddq_du = self.get_Analytical_derivates(
            state, action)
        ...
        for i in range(self.iterations):
            ...
            for n in range(1, N):
                ...
                c_t, C_t, F = self.compute_gradients(self.states[self.planning_horizon-(n+1)],
                                                     self.actions[self.planning_horizon -
                                                                  (n+1)],
            ...
            print(
                f"Iteration number is : {i+1} - Trajectory cost is : {new_cost}   \n")

        t2 = time.time()
        print("total  time  taken:  ", (t2 - t1)*1e+3, "millisecond")

        error = self.states[self.planning_horizon - 1, :] - self.desired_states
    ...
    def plot(self):
        ...
        #    plt.plot(self.actions,linestyle='dashdot',label=("joint_torque1","joint_torque2","joint_torque3"))
            plt.plot(self.actual_effort, linestyle='dashdot', label=(
                "joint_torque1", "joint_torque2", "joint_torque3"))
            plt.ylabel("input N")
            plt.xlabel("time")
            plt.legend()
            plt.figure(figsize=(8, 4))
        #    plt.plot(self.states, linestyle='dashdot',label=("joint_pos1", "joint_pos2", "joint_pos3", "joint_vel1", "joint_vel2", "joint_3"))
            plt.plot(self.actual_states, linestyle='dashdot', label=(
                "joint_pos1", "joint_pos2", "joint_pos3", "joint_vel1", "joint_vel2", "joint_3"))
            plt.ylabel("states")
            plt.xlabel("time")
            plt.legend()
        plt.show()
...
if __name__ == "__main__":
    ...
    test = Trajectory_Optimization(
        initial_pos, planning_horizon, desired_coordinates)
